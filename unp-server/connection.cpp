    ...
    QObject(a_parent)
{
    ...
    connect(&m_process, SIGNAL(errorOccurred(QProcess::ProcessError)), SLOT(processOnErrorOccurred(QProcess::ProcessError)));
    int k = 0;
}
...
void Connection::setSocket(QTcpSocket *a)
{
    ...
    connect(m_socket, SIGNAL(error(QAbstractSocket::SocketError)), SLOT(socketOnError(QAbstractSocket::SocketError)));
    int k = 0;
}
